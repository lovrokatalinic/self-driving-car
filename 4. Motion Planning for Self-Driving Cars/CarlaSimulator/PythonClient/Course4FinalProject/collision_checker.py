    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                # --------------------------------------------------------------
                circle_locations[:, 0] = path[0][j] + self._circle_offsets[0] * np.cos(path[2][j])                
                circle_locations[:, 1] = path[1][j] + self._circle_offsets[1] * np.sin(path[2][j])
                # --------------------------------------------------------------
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                last_index = len(paths[i][0]) - 1
                path_xs = paths[i][0][last_index]
                path_ys = paths[i][1][last_index]
                score = np.sqrt((path_xs - goal_state[0]) ** 2 + (path_ys - goal_state[1]) ** 2)
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                            # --------------------------------------------------
                            path_i_xs = np.array(paths[i][0])
                            path_i_ys = np.array(paths[i][1])
                            path_j_xs = np.array(paths[j][0])
                            path_j_ys = np.array(paths[j][1])
                            paths_distance = np.sqrt((path_i_xs - path_j_xs) ** 2 + (path_i_ys - path_j_ys) ** 2)
                            score += self._weight * np.sum(paths_distance)
                            # --------------------------------------------------
