...
class PathOptimizer:
    ...
    def optimize_spiral(self, xf, yf, tf):
        ...
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        res = scipy.optimize.minimize(fun=self.objective, x0=p0, jac=self.objective_grad, bounds=bounds)
        # ------------------------------------------------------------------
    ...
    def thetaf(self, a, b, c, d, s):
        ...
        params = np.array([a, b/2, c/3, d/4])
        a = np.array(a)
        b = np.array(b) 
        c = np.array(c)
        d = np.array(d)
        s = np.array(s)
        thetas = s*a + (s**2)*(b/2) + (s**3)*(c/3) + (s**4)*(d/4)
        return thetas
        # ------------------------------------------------------------------
