...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    a = C_ns @ imu_f.data[k-1] + g

    p_est[k] = p_est[k-1] + delta_t * v_est[k-1] + 0.5 * (delta_t ** 2) * a
    v_est[k] = v_est[k-1] + delta_t * a
    q_est[k] = Quaternion(axis_angle=imu_w.data[k-1] * delta_t).quat_mult_right(q_est[k-1])
    
    # 1.1 Linearize the motion model and compute Jacobians 
    F_k = np.eye(9)
    F_k[0:3, 3:6] = delta_t * np.eye(3)
    F_k[3:6, 6:9] = -  delta_t * skew_symmetric(C_ns @ imu_f.data[k-1])

    # 2. Propagate uncertainty
    Q_km = np.eye(6)
    Q_km[0:3, 0:3] *= np.eye(3) * (delta_t ** 2) * var_imu_f
    Q_km[3:6, 3:6] *= np.eye(3) * (delta_t ** 2) * var_imu_w
    ...
    if gnss_i < len(gnss.t) and imu_f.t[k] == gnss.t[gnss_i]:
        ...
        gnss_i += 1
        p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_gnss, p_cov[k], gnss_data.T, p_est[k], v_est[k], q_est[k])

    if lidar_i < len(lidar.t) and imu_f.t[k-1] == lidar.t[lidar_i]:
        lidar_data = lidar.data[lidar_i]
        lidar_i += 1
        p_est[k], v_est[k], q_est[k], p_cov[k] = measurement_update(var_lidar, p_cov[k], lidar_data.T, p_est[k], v_est[k], q_est[k])
